    ...
    if (getByTitle.length == 0) {
      ...
      if (getByAuthor.length == 0) {
        ...
        if (getByYear.length == 0) {
          alert(`not found by keyword: ${searchValue.value}`);
        } else {
    ...
    if (book.isCompleted == false) {
      let el = `
            <article class="book_item">
                ...
                <p>Year: ${book.year}</p>
                <div class="action">
                    <button class="green" onclick="readedBook('${book.id}')">read book</button>
                    <button class="yellow" onclick="editBook('${book.id}')">edit book</button>
      ...
      let el = `
            <article class="book_item">
                ...
                <p>Year: ${book.year}</p>
                <div class="action">
                    <button class="green" onclick="unreadedBook('${book.id}')">unread book</button>
                    <button class="yellow" onclick="editBook('${book.id}')">edit book</button>
                    <button class="red" onclick="deleteBook('${book.id}')">delete book</button>
                </div>
            </article>
...
function readedBook(id) {
  let confirmation = confirm("move to read book?");
...
function unreadedBook(id) {
  let confirmation = confirm("move to unread book?");
...
function deleteBook(id) {
  ...
  if (confirmation == true) {
    ...
    localStorage.setItem(localStorageKey, JSON.stringify(bookData));
    showData(getData());
    alert(`the book ${bookDataDetail[0].title} has been deleted`);
  } else {
    return 0;
  }
}
